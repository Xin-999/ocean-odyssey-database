...
-- teams ranked by how many carnivals they appear in
WITH team_usage AS (
    SELECT t.team_name, COUNT(*) AS occ
    FROM   team t
           JOIN carnival c ON c.carn_date = t.carn_date
    WHERE  c.carn_date <= SYSDATE             -- completed only
    GROUP  BY t.team_name
),
popular AS (                                  -- the max count
    SELECT team_name
    FROM   team_usage
    WHERE  occ = (SELECT MAX(occ) FROM team_usage)
),
team_members AS (                             -- pre-count members once
    SELECT t.team_id,
           COUNT(*) AS team_no_members
    FROM   team   t
...
FROM    team t
        JOIN carnival    c  ON c.carn_date = t.carn_date
        JOIN entry       le ON le.event_id = t.event_id
                           AND le.entry_no = t.entry_no  -- leader row
        JOIN competitor  ld ON ld.comp_no = le.comp_no
        JOIN team_members tm ON tm.team_id = t.team_id
        JOIN popular     p  ON p.team_name = t.team_name
ORDER BY t.team_name, c.carn_date;
