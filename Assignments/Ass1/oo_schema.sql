...
COMMENT ON COLUMN ADDRESS.country_code IS 'Country code act as identifier' 
;
...
ALTER TABLE ADDRESS 
    ADD CONSTRAINT ADDRESS_UQ UNIQUE ( add_street_no , add_town , add_postcode , country_code ) ;
...
CREATE TABLE CRUISE 
    ( 
     ...
     cruise_cost            NUMBER (10,2)  NOT NULL , 
     ship_code              NUMBER (7)  NOT NULL 
    ) 
;
...
COMMENT ON COLUMN GENDER.gender_id IS 'Gender identifier' 
;

COMMENT ON COLUMN GENDER.gender_desc IS 'Gender id''s description ' 
;
...
COMMENT ON COLUMN ITINERARY.iti_datetime IS 'Date time of depart or arrival' 
;
...
ALTER TABLE ITINERARY 
    ADD CONSTRAINT ITINERARY_PK PRIMARY KEY ( cruise_id, iti_datetime ) ;

CREATE TABLE MANIFEST 
    ( 
     cruise_id           NUMBER (4)  NOT NULL , 
     pass_id             NUMBER (7)  NOT NULL , 
     mani_board_datetime DATE , 
     cabin_no            VARCHAR2 (7)  NOT NULL , 
     ship_code           NUMBER (7)  NOT NULL 
    ) 
;
...
COMMENT ON COLUMN MANIFEST.mani_board_datetime IS 'Date time when passenger first board the ship' 
;
...
CREATE TABLE OPERATOR 
    ( 
     ...
     oper_ceo_fname VARCHAR2 (50) , 
     oper_ceo_lname VARCHAR2 (50) 
    ) 
;
...
COMMENT ON COLUMN OPERATOR.oper_ceo_fname IS 'First name of ceo of operator' 
;

COMMENT ON COLUMN OPERATOR.oper_ceo_lname IS 'Last  name of ceo of operator' 
;
...
COMMENT ON COLUMN PASSENGER.gender_id IS 'Gender identifier' 
;
...
COMMENT ON COLUMN SHIP.country_code IS 'Country code act as identifier' 
;
